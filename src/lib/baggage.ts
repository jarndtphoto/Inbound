...
export const getBaggage = createServerFn({method:"POST"})
  .validator((input:BaggageLeg)=>{
    if(!input || !/^[A-Z0-9]{2}\d{1,4}[A-Z]?$/.test(input.flight) || !/^[A-Z]{3}$/.test(input.origin) || input.destination!=="HNL" || !/^\d{4}-\d{2}-\d{2}$/.test(input.date))throw new Error("Invalid baggage flight");
