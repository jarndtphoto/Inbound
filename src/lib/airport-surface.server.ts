...
export type SurfaceFeature = {
  id: number;
  kind: "runway" | "taxiway" | "taxiway_area" | "taxilane" | "parking_position" | "apron" | "terminal" | "gate" | "holding_position";
  ref?: string;
  name?: string;
  points: SurfacePoint[];
};
...
export async function loadAirportSurface(input: { airport: string; lat: number; lon: number }): Promise<AirportSurface> {
  ...
  const key = `${airport}:surface-v6:${input.lat.toFixed(3)}:${input.lon.toFixed(3)}`;
